...
function solve(array) {
  ...
  numbers.some((number) => {
    ...
    return boards.every((board, index) => {
      const hasRow = checkRow(board);
      const hasColumn = checkColumn(board);
      
      lastBoard = !hasRow && !hasColumn ? board : lastBoard;

      return hasRow || hasColumn;
    });
  });

  const sumUnmarked = lastBoard.filter(number => number !== -1).reduce((prev, curr) => prev + curr) - lastNumber
